...
impl<'a> TaskParser<'a> {
    ...
    fn parse(input: &[u8]) -> IResult<&[u8], TaskParser> {
        ...
        let start_padding = take_while(|c: u8| !c.is_ascii_digit());
        ...
        let (input, (_, start, _, end, _, jira)) =
            (start_padding, start, time_padding, end, padding, jira_tag).parse(input)?;
        Ok((input, TaskParser { jira, start, end }))
    }

    pub fn parse_activity(input: &[u8]) -> Result<Option<ActivityEntry>> {
        let parse_result = TaskParser::parse(input);

        match parse_result {
            Ok(result) => {
                let start = NaiveTime::parse_from_str(
                    TaskParser::convert_bytes_to_str(result.1.start)?,
                    "%R",
                )?;
                let end = NaiveTime::parse_from_str(
                    TaskParser::convert_bytes_to_str(result.1.end)?,
                    "%R",
                )
                .ok();
...
mod tests {
    ...
    fn test_parse() -> Result<()> {
        let result =
            TaskParser::parse_activity("08:00 - 11:00 JIRA:ABC-123 Some work on task".as_bytes())?;
    ...
    fn test_parse2() -> Result<()> {
        let result =
            TaskParser::parse_activity("08:00 - JIRA:ABC-123 Some work on task".as_bytes())?;
    ...
    #[test]
    fn test_parse_with_list() -> Result<()> {
        let result =
            TaskParser::parse_activity("- [] 08:00 - 11:00 JIRA:ABC-123 Some work on task".as_bytes())?;
